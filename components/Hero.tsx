...
const Hero: React.FC = () => {
  ...
  return (
    <section ref={container} className="pt-16 lg:pt-0 relative h-screen w-full bg-black overflow-hidden flex items-center justify-center">
      {/* Background Layer */}
      <motion.div style={{ y: yVideo }} className="absolute inset-0 z-0">
        ...
        >
          <source src="https://assets.mixkit.co/videos/preview/mixkit-cinematographer-taking-video-with-a-camera-41712-large.mp4" type="video/mp4" />
        </video>
      </motion.div>
      ...
      <div className="absolute bottom-12 left-12 hidden lg:flex flex-col gap-1">
        <span className="text-[8px] font-bold text-zinc-700 tracking-[0.8em] uppercase">Status: Available</span>
        <div className="w-12 h-[1px] bg-zinc-900" />
      </div>

      <div className="absolute bottom-12 right-12 hidden lg:flex flex-col items-end gap-1">
        <span className="text-[8px] font-bold text-zinc-700 tracking-[0.8em] uppercase">Scroll / Discover</span>
        <div className="w-12 h-[1px] bg-zinc-900" />
      </div>
    </section>
  );
